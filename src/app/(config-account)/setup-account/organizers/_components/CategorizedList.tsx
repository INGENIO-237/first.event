...
const CategorizedList: React.FC<CategorizedListProps> = ({ categories, onFollowedChange }) => {
  ...
  const handleDelete = (categoryIndex: number, itemId: number) => {
    const updatedCategories = categoryData.map((category, idx) => {
      if (idx === categoryIndex) {
        const updatedItems = category.items.filter(item => item._id !== itemId);
        return {
          ...category,
          items: updatedItems,
        };
    ...
    <div className="space-y-4 md:w-[80%] overflow-x-hidden">
      {loading ? (
        ...
        categoryData.map((category, categoryIndex) => {
          return (
            <div key={categoryIndex} className="bg-white rounded-lg shadow-md p-4">
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end">
                        ...
                        <motion.button
                          ...
                          whileTap={{ scale: 0.9 }}
                          className={`px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300 ${item.isFollowed
