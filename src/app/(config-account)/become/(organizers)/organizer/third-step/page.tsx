...
const ThirdStep = () => {
    const [targetYearlyEvents, setTargetYearlyEvents] = useState<string>(localStorage.getItem("targetYearlyEvents") ?? "");
            ...
            <div className="grow md:w-1/2 px-5 flex pt-20 pb-5 space-y-5 flex-col justify-around md:items-center">
                ...
                <div className="md:w-5/6 flex flex-col justify- py-9 h-full space-y-5">
                    ...
                    <div className="flex flex-col w-full gap-2">
                        <ExpandRadio id="one" label={"Juste un"} checked={targetYearlyEvents == 'Juste un'} onClick={() => setTargetYearlyEvents('Juste un')} />
                        <ExpandRadio id="two" label={"Quelques-uns (2-12)"} checked={targetYearlyEvents == 'Quelques-uns (2-12)'} onClick={() => setTargetYearlyEvents('Quelques-uns (2-12)')} />
                        <ExpandRadio id="three" label={"Beaucoup (12+)"} checked={targetYearlyEvents == 'Beaucoup (12+)'} onClick={() => setTargetYearlyEvents('Beaucoup (12+)')} />
                        <ExpandRadio id="four" label={"Je ne sais pas encore"} checked={targetYearlyEvents == 'Je ne sais pas encore'} onClick={() => setTargetYearlyEvents('Je ne sais pas encore')} />
                    </div>
                    <div className="flex justify-around w-5/6">
                        ...
                        <button
                            ...
                            className={cn(" rounded py-2 text-center text-white w-2/6 ", targetYearlyEvents == '' ? "cursor-not-allowed bg-gray-400 " : "bg-first_orange hover:bg-orange-600 transition duration-300")} >
                            Suivant</button>
                    </div>
                </div>
            </div>
