...
const FirstStep = () => {
  ...
  if (condition) {
    localStorage.setItem(
      "redirect-after-setup-account",
      "/become/organizer/step-1"
    );
      ...
      <div className="grow md:w-1/2 px-5 flex pt-20 pb-5 space-y-5 flex-col justify-around md:items-center">
        <div className="md:w-5/6 flex flex-col justify- py-9 h-full space-y-5">
          ...
          <div className="flex flex-col w-full gap-2">
            <ExpandRadio
              id="first-time"
              label={"Je n'ai pas encore organisé d'événement"}
              checked={experience == "Je n'ai pas encore organisé d'événement"}
              onClick={() =>
                setExperience("Je n'ai pas encore organisé d'événement")
              }
            />
            <ExpandRadio
              id="little-exp"
              label={"J'ai déjà organisé un événement"}
              checked={experience == "J'ai déjà organisé un événement"}
              onClick={() => setExperience("J'ai déjà organisé un événement")}
            />
            <ExpandRadio
              id="big-exp"
              label={"J'ai beaucoup d'expérience en organisation d'événements"}
              checked={
                experience ==
                "J'ai beaucoup d'expérience en organisation d'événements"
              }
              onClick={() =>
                setExperience(
                  "J'ai beaucoup d'expérience en organisation d'événements"
